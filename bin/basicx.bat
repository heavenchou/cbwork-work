...
# 2011/12/04 by heaven : <app> 中沒有文字的部份要換成 &lac;
# 2011/12/05 by heaven : <T> 標記不要中止 list 及 item .
# 2012/01/04 by heaven : <app> 移位 沒有文字的部份要換成 &lac;
#
#-------------------------------------------------------------------------------------------------
#$xml_root="/cbwork/xml";
...
sub corr2app {
	...
	while ($text =~ /^((?:$big5)*)\[($big5a*?)>>($big5a*?)\](.*)$/) {
		...
		if($lem eq "") {$lem = "&lac;"};	# 2012/01/04 沒有文字時要用 &lac;
		if($rdg eq "") {$rdg = "&lac;"};
		
		$text = $1 . "<app type=\"shift\"><lem wit=\"【CBETA】\" resp=\"CBETA.maha\">$lem</lem><rdg wit=\"$wit\">$rdg</rdg></app>" . $4;
	}
	# 修訂 [a>b]
	while ($text =~ /^((?:$big5)*)\[($big5a*?)>($big5a*?)\](.*)$/) {
		if ($debug3) {
