...
# my $source_root = "C:/cbwork/simple/release/normal/";		# fn1.來源目錄

# my $source_root = "C:/release/normal-des/";				# fn2.來源目錄
# my $source_root = "C:/release/app1-des/";					# fn2.來源目錄

# my $source_root = "C:/release/normal-des/";				# fn3.來源目錄
# my $source_root = "C:/release/app1-des/";					# fn3.來源目錄

my $source_root = "C:/cbwork/xml/";						# fn3.來源目錄

# my $source_root = "C:/cbwork/simple/release/normal/";		# fn3.來源目錄, 台北版用的, $function = "big52utf8"


# my $target_root = "c:/release/normal/";			# fn1.目的目錄

# my $target_root = "c:/release/normal-des-nor/";	# fn2.目的目錄
# my $target_root = "c:/release/app1-des-nor/";		# fn2.目的目錄

# my $target_root = "c:/release/normal-des-utf8/";	# fn3.目的目錄
# my $target_root = "c:/release/app1-des-utf8/";	# fn3.目的目錄

my $target_root = "c:/release/xml-utf8/";			# fn3.目的目錄

# my $target_root = "C:/cbwork/simple/release/normal/utf8out/";		# fn3.目的目錄, 台北版用的, $function = "big52utf8"

my $file_ext = "*.*";								# 要處理的副檔名

my $TFrom = 14;		# 大正藏處理冊數開始 1
my $Tto = 85;		# 大正藏處理冊數結束 85
my $XFrom = 1;		# 卍續藏處理冊數開始 1
my $Xto = 88;		# 卍續藏處理冊數結束 88
my $JFrom = 1;		# 嘉興藏處理冊數開始 1
my $Jto = 40;		# 嘉興藏處理冊數結束 40
my $HFrom = 1;		# 正史處理冊數開始 1
my $Hto = 1;		# 正史處理冊數結束 1
my $WFrom = 1;		# 藏外處理冊數開始 1
my $Wto = 9;		# 藏外處理冊數結束 3
...
my $run_no_vol = 0;		# 沒有子目錄, 直接在指定目錄處理

# 1. 二組檔案比對 專用變數

my $difffile = "2filediff.txt";		# 功能 1 的差異檔

my $skip_fullspace = 1;				# 忽略全型空白
...
my $skip_kbj = 1;					# 卍續藏的 【科08】【標08】【解08】, 未來普及版應該不會有這個

# 2. 組字式轉成通用字 專用變數
# 3. big5 轉成 utf8 專用變數

my $des2uni = 0;					# 是否要將缺字換成 unicode ? (xml 不用, normal, app 要)
my $des2nor = 0;					# 是否要將缺字換成通用字 ? (xml 不用, normal, app 要)(T54n2128 及 2129 會自動取消)
...
require "ranjana.plx"  if($function eq "utf82big5");  ## this is needed for handling the big5 entity replacements
	...
	elsif ($function eq "comp2file")	
	{
		...
		while(<IN>)
		{
			...
			if($_ ne $line)
			{
				...
				if($skip_kbj)	# 卍續藏的 【科08】【標08】【解08】, 未來普及版應該不會有這個
				{
					$line1 =~ s/【((科)|(標)|(解))\d+】//g;
					$line2 =~ s/【((科)|(標)|(解))\d+】//g;
				}
...
sub big52utf8ln
{
	...
	$line =~ s/encoding="cp950"/encoding="UTF-8"/;		# xml 版
	$line =~ s/coding: big5-dos/charset: CP950/;		# xml 版
	$line =~ s/\(Big5\)/\(UTF-8\)/i;
	$line =~ s/\(CP950\)/\(UTF-8\)/i;
	my $result = "";
...
sub utf82big5ln
{
	...
	while($line =~ /^($utf8)/)
	{
		...
		{			
			...
			#if(exists $rj2dia{$c})
			#{
			#	$tmp = cbdia2smdia($rj2dia{$c});		# 蘭札體
			#}
			#els
			if(exists $sd2dia{$c})
			{
				$tmp = cbdia2smdia($sd2dia{$c});		# 悉曇體
			}
			else
			{
				$tmp = "&$tmp;";
	...
	if($remove_linehead_eng)
	{
		...
		if($easyver)
		{
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d).(\d\d).*?║//i;
		}
		else
		{
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)a(\d\d).*?║/$1_$2_$3_1_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)b(\d\d).*?║/$1_$2_$3_2_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)c(\d\d).*?║/$1_$2_$3_3_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)d(\d\d).*?║/$1_$2_$3_4_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)e(\d\d).*?║/$1_$2_$3_5_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)f(\d\d).*?║/$1_$2_$3_6_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)g(\d\d).*?║/$1_$2_$3_7_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)h(\d\d).*?║/$1_$2_$3_8_$4/i;
			$result =~ s/^[TXJHWI](\d\d)n(.\d\d\d).p(\d\d\d\d)i(\d\d).*?║/$1_$2_$3_9_$4/i;
		}
	}
	if($easyver)
	{
